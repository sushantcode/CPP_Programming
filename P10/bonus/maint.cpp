    ...
    do {
        ...
        if(choice == 1) {
			...
			do{
		        ...
		        std::cout << "\n" << "Do you want to add more polynomials(If yes, enter y/Y): ";
		        std::cin >> res; std::cin.ignore(32767, '\n');
			}while(res == 'y' || res == 'Y');
        } else if(choice == 2) {
        	for (auto& po : polynomials){
		        std::cout << "Filename: ";
		        std::getline(std::cin, filename);
		        if(filename.size()) {
		             try {
		                ...
		                po.save(ofs);
		                std::cout << "Saved " << filename << ": " << po << std::endl;
		            } catch(std::exception& e) {
            ...
            for (i = 0; i < polynomials.size(); ++i){
            	Polynomial p = polynomials.at(i);
		        t.push_back(new std::thread{&Polynomial::solve, &p, c, e, nthreads});
		    }
		    for (i = 0; i < polynomials.size(); ++i){
		    	(t.at(i))->join();
		    }
