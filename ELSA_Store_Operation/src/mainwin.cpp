...
Mainwin::Mainwin(){
    ...
    //adding insert Ram to Insert peripheral menu
...
void Mainwin::on_open_click(){
    ...
    if (result == 1) {
    	try {
            ...
            set_msg("Opened file: " + filename);
        } catch (std::exception& e) {
        	std::string errMsg = "Unable to open the file: " + filename + ", " + e.what() + ".";
        	set_msg("Error: " + errMsg);
            Gtk::MessageDialog{*this, errMsg, false, Gtk::MESSAGE_WARNING}.run();
            on_new_store_click();
        }
    }
}
...
void Mainwin::on_insert_ram_click(){
	...
	Ram option{e_name.get_text(), cost, size};
...
void Mainwin::on_insert_cpu_click(){
	...
	CPU option{e_name.get_text(), cost, frequency};
	store->add_option(option);
	set_msg("Added new peripheral");
	on_view_peripheral_click();
}
...
void Mainwin::on_insert_disk_click(){
	...
	Disk option{e_name.get_text(), cost, size};
	store->add_option(option);
	set_msg("Added new peripheral");
	on_view_peripheral_click();
}
...
void Mainwin::on_insert_other_click(){
	Gtk::Dialog dialog{"Add Other Peripheral", *this};
	Gtk::Grid grid;
